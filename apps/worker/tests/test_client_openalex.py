...
from biblio_checker_worker.langgraph.clients.openalex import (
    OPENALEX_BASE_URL,
    OpenAlexClient,
)
...
class TestOpenAlexDoiLookup:
    def test_doi_found_returns_single_candidate(self) -> None:
        ...
        client = _make_client()
        with patch.object(
            client._client, "get", return_value=_mock_response(200, json_body=work)
        ):
    ...
    def test_invalid_doi_format_skips_lookup_falls_through_to_title(self) -> None:
        ...
        with patch.object(
            client._client, "get", return_value=title_response
        ) as mock_get:
            results = client.search(
                title="Some Title",
                authors=[],
    ...
    def test_doi_url_path_encodes_doi(self) -> None:
        ...
        assert ")" not in captured_calls[0]
        assert (
            "%28" in captured_calls[0]
            or "%2528" in captured_calls[0]
            or "ex%28ample%29" in captured_calls[0]
        )
...
class TestOpenAlexTitleSearch:
    ...
    def test_title_search_no_results_returns_empty_list(self) -> None:
        ...
        with patch.object(
            client._client,
            "get",
            return_value=_mock_response(200, json_body={"results": []}),
        ):
            results = client.search(
                title="Completely Obscure",
                authors=[],
                year=None,
                doi=None,
                arxiv_id=None,
...
class TestOpenAlexAuthorTitleSearch:
    def test_author_title_search_triggered_when_title_and_authors_provided(
        self,
    ) -> None:
        ...
        responses = iter(
            [
                _mock_response(
                    200, json_body=results_body
                ),  # author+title search finds result
            ]
        )
